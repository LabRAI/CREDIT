...
class FeatureBackdoorDataset(torch.utils.data.Dataset):
    def __init__(self, base_dataset, feat_cols, target_label, seed):
        self.base_dataset = base_dataset
        self.feat_cols = int(max(0, feat_cols))
    ...
    def __getitem__(self, idx):
        ...
        if hasattr(data, "x") and data.x is not None:
            ...
            if not torch.is_floating_point(x):
                x = x.float()
            C = x.size(-1)
            k = min(self.feat_cols, C) if self.feat_cols > 0 else 0
            if k > 0:
                g = torch.Generator(device=x.device if x.is_cuda else "cpu")
                g.manual_seed(self.seed + idx)
                rand_block = torch.rand(x.size(0), k, generator=g, device=x.device, dtype=x.dtype)
                x = x.clone()
                x[:, C - k: C] = rand_block
            data.x = x

        else:
            raise ValueError("Graph sample has no node features x, cannot apply feature backdoor.")

        data.y = torch.tensor(self.target_label, dtype=torch.long)

        return data


class BackdoorWM(nn.Module):
    def __init__(
            self,
    ...
    def _key_gen(self):
        ...
        new_wm_dataset = FeatureBackdoorDataset(
            wm_dataset,
            feat_cols=self.wm_feat_len,
            target_label=self.wm_target_label,
            seed=self.seed,
        )
        ...
        print(
            f"[KeyGen] Applied feature backdoor on last {self.wm_feat_len} feature columns "
            f"and set wm label = {self.wm_target_label} for {len(new_wm_dataset)} graphs."
        )
    ...
    def _finetune_with_backdoor(self, model, epochs, lr, weight_decay):
        ...
        for epoch in range(epochs):
            ...
            print(
                f"[BackdoorWM]train epoch: {epoch}, loss: {loss_sum / total}, wm acc: {acc_sum / total}")
        ...
        for x, y in self._test_loader:
            ...
            loss_sum += loss.item() * bs
            acc_sum += (logits.argmax(1) == y).float().sum().item()
        print(f"[BackdoorWM]test loss: {loss_sum / total}, test acc: {acc_sum / total}")
    ...
    def eval_downstream(self):
        ...
        for x, y in self._test_loader:
            ...
            acc_sum += (logits.argmax(1) == y).float().sum().item()
        acc = acc_sum / total
        print(f"[BackdoorWM]Downstream acc: {acc:.6f}")
        return acc
    ...
    def eval_wm(self):
        ...
        for x, y in self._wm_loader:
            ...
            acc_sum += (logits.argmax(1) == y).float().sum().item()
        acc = acc_sum / total
        print(f"[BackdoorWM]WM acc: {acc:.6f}")
        return acc
